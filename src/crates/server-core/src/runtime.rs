...
impl AsyncRuntime {
    ...
    pub fn new(config: &norc_config::ResourceLimits) -> Result<Self, ServerError> {
        ...
        eprintln!("Initializing async runtime with {} CPU cores detected", cpu_cores);
...
impl RuntimeMetrics {
    ...
    fn increment_spawned(&self, _workload: WorkloadType) {
        self.tasks_spawned.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    }
    
    fn increment_blocking_spawned(&self, _workload: WorkloadType) {
        self.blocking_tasks_spawned.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    }
    
    /// Get total tasks spawned
    pub fn tasks_spawned(&self) -> u64 {
        self.tasks_spawned.load(std::sync::atomic::Ordering::Relaxed)
    }
    
    /// Get total blocking tasks spawned
    pub fn blocking_tasks_spawned(&self) -> u64 {
        self.blocking_tasks_spawned.load(std::sync::atomic::Ordering::Relaxed)
    }
}
...
mod tests {
    ...
    fn test_spawn_task() {
        ...
        // Spawn a simple task
        let handle = runtime.spawn(WorkloadType::Business, async {
            42
        });
