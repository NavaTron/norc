...
async fn test_revocation_checker_creation() {
    ...
    assert!(checker.is_ok(), "Should create RevocationChecker successfully");
}
...
async fn test_check_revocation_with_no_urls() {
    ...
    let cert = create_client_cert("TestOrg", "client-001", None)
        .expect("Failed to create test cert");
...
async fn test_check_revocation_timeout() {
    ...
    assert!(result.is_ok() || result.is_err(), "Should handle no URLs gracefully");
}
...
async fn test_fail_on_unknown_false() {
    ...
    let checker_allow = RevocationChecker::new(config_allow)
        .expect("Failed to create checker");
    
    let result_allow = checker_allow.check_revocation(&cert.as_rustls_cert(), None).await;
    
    // With fail_on_unknown=false, should succeed even on errors
    assert!(result_allow.is_ok(), "Should return Ok when fail_on_unknown=false");
}
...
async fn test_fail_on_unknown_true() {
    ...
    let checker_fail = RevocationChecker::new(config_fail)
        .expect("Failed to create checker");
    
    let result_fail = checker_fail.check_revocation(&cert.as_rustls_cert(), None).await;
...
async fn test_revocation_checker_with_expired_cert() {
    let expired_cert = create_expired_cert("TestOrg")
    ...
    let result = checker.check_revocation(&expired_cert.as_rustls_cert(), None).await;
    
    // Revocation checking should work regardless of cert expiration
    // The expiration check is separate from revocation checking
    assert!(result.is_ok() || result.is_err(), "Should handle expired cert");
}
...
async fn test_ocsp_disabled() {
    ...
    assert!(result.is_ok() || result.is_err(), "Should complete without OCSP");
}
...
async fn test_crl_disabled() {
    ...
    assert!(result.is_ok() || result.is_err(), "Should complete without CRL");
}
...
async fn test_check_with_issuer_cert() {
    ...
    let result = checker.check_revocation(
        &client.as_rustls_cert(),
        Some(&ca.as_rustls_cert())
    ).await;
...
async fn test_multiple_certs_different_orgs() {
    ...
    let cert1 = create_client_cert("OrgA", "client-001", None)
        .expect("Failed to create cert 1");
    let cert2 = create_client_cert("OrgB", "client-002", None)
        .expect("Failed to create cert 2");
    let cert3 = create_client_cert("OrgC", "client-003", None)
        .expect("Failed to create cert 3");
    
    // Check all certificates
    let result1 = checker.check_revocation(&cert1.as_rustls_cert(), None).await;
    let result2 = checker.check_revocation(&cert2.as_rustls_cert(), None).await;
    let result3 = checker.check_revocation(&cert3.as_rustls_cert(), None).await;
