...
use norc_transport::rotation_manager::{
    CertificateBundle, RotationConfig, RotationManagerBuilder,
};
use std::sync::Arc;
...
async fn create_temp_cert_files() -> (String, String) {
    ...
    let (cert_file, key_file) = write_cert_to_temp_files(&cert_bundle)
        .expect("Failed to write cert to temp files");
...
async fn test_certificate_bundle_load() {
    ...
    assert!(!bundle.certs.is_empty(), "Certificate chain should not be empty");
...
async fn test_certificate_bundle_reload() {
    ...
    assert!(bundle.loaded_at > original_loaded_at, "loaded_at should be updated after reload");
    assert!(bundle.age() < Duration::from_secs(1), "Bundle age should be small after reload");
}
...
async fn test_certificate_bundle_age() {
    ...
    assert!(new_age > initial_age, "Bundle age should increase over time");
    assert!(new_age >= Duration::from_millis(200), "Age should reflect elapsed time");
}
...
async fn test_builder_missing_cert_path() {
    ...
    assert!(err_msg.contains("Certificate path"), "Error should mention certificate path: {}", err_msg);
}
...
async fn test_builder_missing_key_path() {
    ...
    let err_msg = result.err().unwrap().to_string();
    assert!(err_msg.contains("Key path"), "Error should mention key path: {}", err_msg);
}
...
async fn test_manual_reload_basic() {
    ...
    assert!(new_bundle.loaded_at > original_loaded_at, "Bundle should be reloaded");
}
...
async fn test_manual_reload_cooldown_enforcement() {
    ...
    manager.reload().await.expect("Second reload should not error");
...
async fn test_manual_reload_multiple_times() {
    ...
    for _ in 0..3 {
        ...
        assert!(current_loaded_at > previous_loaded_at, "Each reload should update loaded_at");
...
async fn test_subscribe_receives_notifications() {
    ...
    assert!(new_bundle.loaded_at > initial_loaded_at, "Notification should contain updated bundle");
}
...
async fn test_reload_after_file_deletion() {
    ...
    assert!(result.is_err(), "Reload should fail when cert file is deleted");
}
...
async fn test_reload_with_invalid_cert_content() {
    ...
    // Reload should fail
    let result = manager.reload().await;
    assert!(result.is_err(), "Reload should fail with invalid cert content");
}
...
async fn test_current_bundle_returns_latest() {
    ...
    assert!(loaded_at2 > loaded_at1, "current_bundle should return the latest bundle");
}
    ...
    for i in 0..5 {
        ...
        let handle = std::thread::spawn(move || {
            ...
            runtime.block_on(async move {
                for _ in 0..10 {
                    let bundle = mgr.current_bundle().await;
                    assert!(!bundle.certs.is_empty(), "Thread {} should get valid bundle", i);
