...
pub async fn list_devices(
    ...
    for user in users {
        let user_devices = state.device_repo.find_by_user(&user.id).await?;
    ...
    let device_responses: Vec<DeviceResponse> =
        all_devices.into_iter().map(device_to_response).collect();
...
pub async fn get_device(
    ...
    let device = state.device_repo.find_by_id(&device_id.to_string()).await?;
