...
pub async fn list_sessions(
    ...
    let sessions_result = if let Some(user_id) = params.user_id {
        // Filter by user ID
        state.session_repo.list_by_user(&user_id.to_string(), Some(params.limit.unwrap_or(50))).await
    } else if let Some(device_id) = params.device_id {
        // Filter by device ID
        state.session_repo.list_by_device(&device_id.to_string(), Some(params.limit.unwrap_or(50))).await
    } else {
        // Get all active sessions
        state.session_repo.list_active(Some(params.limit.unwrap_or(50))).await
    };
    
    let sessions = sessions_result.map_err(|e| {
        crate::ApiError::Internal(format!("Failed to query sessions: {}", e))
    })?;
    ...
    let session_responses: Vec<SessionResponse> = sessions
        ...
        .map(|session| {
            ...
            SessionResponse {
                ...
                device_name: session.device_name.unwrap_or_else(|| "Unknown Device".to_string()),
                state: if session.expires_at > Utc::now() { "active".to_string() } else { "expired".to_string() },
...
pub async fn revoke_user_sessions(
    ...
    let revoked_count = state.session_repo
